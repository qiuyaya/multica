...
VALUES ($1, $2, $3, $4, $5)
RETURNING id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at
`
...
func (q *Queries) DeleteSquad(ctx context.Context, id pgtype.UUID) error {
	_, err := q.db.Exec(ctx, deleteSquad, id)
	return err
}

const getSquad = `-- name: GetSquad :one
SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at FROM squad WHERE id = $1
`
...
SELECT s.id, s.workspace_id, s.name, s.description, s.leader_id, s.creator_id, s.created_at, s.updated_at FROM squad s WHERE s.id = $1 AND s.workspace_id = $2
`
...
SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at FROM squad WHERE id = $1 AND workspace_id = $2
`
...
SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at FROM squad WHERE workspace_id = $1 ORDER BY created_at ASC
`
...
SELECT s.id, s.workspace_id, s.name, s.description, s.leader_id, s.creator_id, s.created_at, s.updated_at FROM squad s
